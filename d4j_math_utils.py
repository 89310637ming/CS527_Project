...
MATH_ROOT = D4J_ROOT / "framework" / "projects" / "Math"
COMMONS_MATH_REPO = PROJECT_ROOT / "project_repos" / "commons-math"
...
def parse_active_bugs() -> pd.DataFrame:
    path = MATH_ROOT / "active-bugs.csv"
...
def load_pilot_bug_ids() -> List[int]:
    selection_path = PROJECT_ROOT / "math_bug_selection.csv"
...
def parse_trigger_stack_classes(path: Path) -> List[str]:
    ...
    for line in read_nonempty_lines(path):
        ...
        if class_name.startswith("org.apache.commons.math3"):
...
def parse_src_patch_stats(bug_id: int) -> Dict[str, PatchStats]:
    patch_path = MATH_ROOT / "patches" / f"{bug_id}.src.patch"
...
def parse_modified_classes(bug_id: int) -> List[str]:
    path = MATH_ROOT / "modified_classes" / f"{bug_id}.src"
...
def git_output(*args: str) -> str:
    result = subprocess.run(
        ["git", "-C", str(COMMONS_MATH_REPO), *args],
        check=True,
        capture_output=True,
        text=True,
    )
...
def build_bug_metadata(bug_ids: Iterable[int]) -> pd.DataFrame:
    ...
    for bug_id in bug_ids:
        ...
        relevant_tests = read_nonempty_lines(MATH_ROOT / "relevant_tests" / str(bug_id))
        trigger_path = MATH_ROOT / "trigger_tests" / str(bug_id)
