...
def main() -> None:
    ...
    bug_metadata.to_csv(data_dir / "math_bug_metadata.csv", index=False)

    test_instances = build_test_instances(bug_metadata)
    test_instances.to_csv(data_dir / "math_test_instances.csv", index=False)
    ...
    features.to_csv(data_dir / "math_test_instances_features.csv", index=False)

    ranked, apfd = run_all_methods(features)
    ranked.to_csv(data_dir / "math_ranked_tests.csv", index=False)
    apfd.to_csv(data_dir / "math_apfd_results.csv", index=False)

    summary = apfd.groupby("method", as_index=False)["apfd"].agg(["mean", "median"]).reset_index()
    print("Math experiment complete")
